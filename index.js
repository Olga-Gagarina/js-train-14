...
function nextNewYear() {
  const now = new Date();
  const stillYear = now.getFullYear();
  const nextYear = stillYear + 1 ;
  const nextNewYearDate = new Date( nextYear, 0 ,1);
  const timeDifference = nextNewYearDate - now;
 ...
 */
  ...
  function isLeapYear(year) {
    if (isNaN(year)) {
      return { year, isLeap: false, error: "Помилка, вхідне значення має бути числом" };
    }
...
function getDaysInMonth(date) {
  ...
  const lastDateMonth = new Date (nextMonthDate - 1 );
  const daysInMonth = lastDateMonth.getDate();
  return {
    inputDate , daysInMonth
  };
  // Перевірка, чи є вхідне значення об'єктом Date,це можно зробити перевіривши чи є date.getTime по типу функція .
  // Якщо date не є об'єктом Date, повертаємо рядок
  // "Помилка: вхідне значення має бути об'єктом Date"
...
function getFormattedTime(date) {
  ...
  const formateTime =` ${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`;
  return {
    inputDate,
    formateTime
  };
...
function getAge(birthDate) {
  ...
  inputDate.getMonth() === birthDate.getMonth &&
  inputDate.getDate() === birthDate.getDate
  ){
   age -1 ;
  }
...
function compareDates(date1, date2) {
  ...
  else if ( date1 === date2) { comparison = 0}
  else  {  comparison =  1};
...
function getDaysDifference(startDate, endDate) {
  ...
  const endDate2 = endDate.toISOString(); 
  return {
